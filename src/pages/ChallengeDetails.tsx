...
const ChallengeDetails: React.FC = () => {
    ...
    const { title, startDate, description, level } = location.state as {
        title: string,
        startDate: Date,
        description: string,
        level: string,
            ...
            <div className="h-12 border px-24">
                <div className="h-full flex items-end justify-between">
                    ...
                    <div className="self-center">
                        <button className="bg-[#44924C] py-1 px-4 rounded-lg text-white font-semibold mx-2">Edit</button>
                        <button onClick={() => handleDelete(title)} className="py-1 px-4 rounded-lg text-red-600 border border-red-600 mx-2">Delete</button>
                    </div>
                </div>
